...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    """Проверьте свойства функции сигмоиды, в частности:
    1 Она всегда находится между 0.0 и 1.0.
    2 Один минус сигмоида равен сигмоиде от отрицательного аргумента.
    3 Она пересекает 0 в точке 0.5.
    4 Она строго возрастает."""

    # 1 Она всегда находится между 0.0 и 1.0.
    assert 0.0 <= sigmoid(a) <= 1.0
    # 2 Один минус сигмоида равен сигмоиде от отрицательного аргумента.
    # assert abs(sigmoid(a) - (1 - sigmoid(-a))) < 1e-10
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    """Проверьте транзитивность оператора «меньше», а именно:
    Если a < b и b < c, то должно выполняться a < c.
    """
    if a < b and b < c:
        assert a < c
...
def test_symmetric(a: float, b: float) -> None:
    ...
    """
    """Проверьте, что функция умножения :func:`minitorch.operators.mul` симметрична, а именно:
    Результат умножения не зависит от порядка аргументов (a * b == b * a).
    """
...
def test_distribute(z: float, x: float, y: float) -> None:
    ...
    """
    """Проверьте дистрибутивность умножения над сложением, а именно:
    z * (x + y) должно быть равно z * x + z * y.
    """
    left = mul(z, add(x, y))
...
def test_other(x: float, y: float, z: float) -> None:
    ...
    """
    """Проверьте другое свойство операторов, например:
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    """Проверьте, что сумма ls1 плюс сумма ls2 равна сумме поэлементных сумм ls1[i] + ls2[i]."""
    left = sum(ls1) + sum(ls2)
    right = sum([add(x, y) for x, y in zip(ls1, ls2)])
    assert_close(left, right)
